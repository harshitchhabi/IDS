...
Per round: generate batch -> auto-label -> defense ``apply`` -> retain -> defense
``sanitize`` -> cold-start retrain -> evaluate on ``trusted_eval`` (both threshold
modes, per-arm TPR).
...
* A defense may change only the weights of honeypot-derived rows. seed_train is
  trusted and the loop asserts it is untouched.
"""
...
from dloop.loop.labeler import auto_label, ground_truth_label
...
def run_arm(
    ...
    *,
    ...
    label_fn = auto_label if config.label_policy == "auto_malicious" else ground_truth_label
    ...
    for k in range(config.rounds + 1):
        ...
        w_train = (np.concatenate([np.ones(n_seed)] + [b.w for b in retained])
                   if retained else np.ones(n_seed))
        is_hp = np.arange(len(y_train)) >= n_seed

        if n_hp:
            t0 = time.perf_counter()
            w_new = np.asarray(defense.sanitize(TrainingView(x_train, y_train, w_train, is_hp, k, seed)),
                               dtype="float64")
            defense_s += time.perf_counter() - t0
            if w_new.shape != w_train.shape or (w_new < 0).any():
                raise ValueError("defense.sanitize must return non-negative weights of the training-set shape")
            if not np.array_equal(w_new[:n_seed], w_train[:n_seed]):
        ...
        cfg = ModelConfig(kind=model_kind, seed=seed * 1000 + k, target_fpr=config.target_fpr,
                          hyperparams=hp, val_min_nn_distance=config.val_min_nn_distance)
        t0 = time.perf_counter()
        ...
        for mode, thr in (("fixed", thr_fixed), ("recalibrated", thr_recal)):
            ...
            row = {
                ...
                "label_policy": config.label_policy, "defense": defense.name,
                "val_min_nn_distance": config.val_min_nn_distance,
                "val_benign_dropped_frac": model.val_benign_dropped_frac_,
                ...
                "hp_mean_weight": float(hp_w.mean()) if n_hp else float("nan"),
                "hp_zero_weight_frac": float((hp_w == 0).mean()) if n_hp else float("nan"),
                "hp_effective_ratio": float(hp_w.sum() / w_train.sum()) if n_hp else 0.0,
                "defense_seconds": defense_s, "fit_seconds": fit_s,
