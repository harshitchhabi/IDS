...
\begin{abstract}
% Source: docs/PAPER_PLAN.md "The claim, in one paragraph", corrected per DECISIONS.md s26.1/s26.5.
...
a recalibrated one surfaces as lost detection, not two independent effects. The attack needs
near-duplicate mimicry fidelity and at least 10--20\% of the training set poisoned; after
Holm-Bonferroni correction across the full grid we test, both models we evaluate show
detection-loss damage at that same fidelity level, and a separate, pre-specified result at
greater mimicry distance is reported honestly as an effect found but not yet confirmed to
generalise, rather than rounded up to a second channel. A generic label-consistency defense
