...
around a mean (Section~\ref{sec:attack}), and RF's apparent damage at large mimicry distance
remains unconfirmed at this seed count after correcting for the number of comparisons made
(Section~\ref{sec:attack}). ShareCap's safe cap range is measured for one model and one dataset
at a time and is not a universal constant; we do not test an adaptive attacker who tunes its
poison ratio specifically to fill the cap rather than exceed it.
